...
function Dropdown({ label, options = [], onChange, initial = null, buttonClass = '' }: DropdownProps) {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((s) => !s)}
        className={`min-w-[160px] flex items-center justify-between px-5 py-3 bg-[#0b63c9] text-white rounded-md border border-white/20 shadow-inner ${buttonClass}`}
      >
        <span className="truncate">{value}</span>
        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 ml-3 opacity-80" viewBox="0 0 20 20" fill="currentColor">
          <path fillRule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 011.08 1.04l-4.25 4.25a.75.75 0 01-1.08 0L5.21 8.27a.75.75 0 01.02-1.06z" clipRule="evenodd" />
        </svg>
      </button>

      {open && (
        <ul className="absolute left-0 mt-2 w-56 bg-white text-gray-800 rounded shadow-lg overflow-hidden z-30">
          {options.map((o) => (
            <li key={o}>
              <button
                onClick={() => select(o)}
  ...
  return (
    <section className="relative">
            <div className="relative h-96 md:h-[520px] w-full z-0">
              ...
              />
              {/* overlay to ensure readable text on top of image */}
            ...
            {/* Overlapping search card (full width centered) */}
            <div className="-mt-16 relative z-20">
              <div className="max-w-6xl mx-auto px-6">
                <div className="bg-white rounded-xl shadow-2xl p-4 flex items-center gap-4">
                  ...
                  <input
                    className="flex-1 px-4 py-4 rounded-md focus:outline-none text-gray-700"
            ...
            <div className="bg-[#0b63c9] mt-6">
              <div className="max-w-6xl mx-auto px-6 py-6">
                <div className="flex flex-wrap gap-4 items-center">
                  ...
                  <Dropdown
                    label="Min Price"
                    options={["Any", "R 100 000", "R 150 000", "R 200 000", "R 250 000", "R 300 000", "R 350 000", "R 400 000", "Custom Price"]}
                    initial={minPrice}
                  ...
                  <Dropdown
                    label="Max Price"
                    options={["Any", "R 500 000", "R 750 000", "R 1 000 000", "R 1 500 000", "R 2 000 000"]}
                    initial={maxPrice}
